...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    let rpc_addr = "127.0.0.1:6682"; // TODO: Load from config
                                     //RPC server for handling rpc commands
    if let Some(rpc_command) = args.command {
        ...
        let _parsing_handle = tokio::spawn(parse_arguments(rpc_command, socket_address)).await;
    } else {
    ...
    //Graceful shutdown handling upon interrupt
